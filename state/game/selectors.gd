...
static func getFieldType(state: TTT_State, index: int, parentIndex: int = TTT_State.mainFieldIndex) -> TTT_State.FieldType:
	if parentIndex == TTT_State.mainFieldIndex: return state.fields[index].type

	if state.fields[parentIndex].type == TTT_State.FieldType.field:
		return state.fields[parentIndex].inner[index].type

	assert("Cannot find field [" + str(parentIndex) + "][" + str(index) + "]. It\"s not field cell.")
	return TTT_State.FieldType.none

static func getFieldList(state: TTT_State, parentIndex: int) -> Array:
	return state.fields if parentIndex == TTT_State.mainFieldIndex else state.fields[parentIndex].inner

static func _getIsFieldFull(fieldList: Array) -> bool:
	for cell in fieldList:
		if cell.type == TTT_State.FieldType.none:
			return false
	return true
